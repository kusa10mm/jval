...
describe('test baseValidate() function', () => {
   it('should call validators with value and conditions', () => {
       ...
       expect(mocked(validateRequired)).toBeCalledWith('hello', conditions);
       expect(mocked(validateNullable)).toBeCalledWith('hello', conditions);
       expect(mocked(validateOneOf)).toBeCalledWith('hello', conditions);
    })
});